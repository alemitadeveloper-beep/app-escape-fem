...
class WordDatabase {
  ...
  Future<void> importEscapesFromScrapedJson() async {
    ...
    for (final e in data) {
      ...
      // Limpiar imagen URL
      final imageUrl = m['imagenUrl']?.toString().trim();
      if (imageUrl != null && imageUrl.isNotEmpty && imageUrl.startsWith('http')) {
        m['imagenUrl'] = imageUrl;
        print('📸 Imagen encontrada: ${nombre.substring(0, nombre.length < 30 ? nombre.length : 30)} -> ${imageUrl.substring(0, imageUrl.length < 50 ? imageUrl.length : 50)}...');
      } else {
        m['imagenUrl'] = null;
      }
